    ...
    public class UI_TextComplex
    {
        ...
        )
        {
            ...
            while (tmp.IndexOf(iconChar) != -1)
            {
                ...
                tmp = tmp.Substring(
                    tmp.IndexOf(iconChar + iconNumber) + (iconChar + iconNumber).Length
                );
                if (untilTmp.Trim() != "")
                {
                    Text uiText = UtilsClass.DrawTextUI(
                        untilTmp,
                        transform,
                        new Vector2(textPosition, 0),
                        fontSize,
                        font
                    );
                ...
                UtilsClass.DrawSprite(
                    icon.sprite,
                    transform,
                    new Vector2(textPosition + icon.size.x / 2f, 0),
                    icon.size
                );
